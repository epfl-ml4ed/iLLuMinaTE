...
def call_groq_model_with_retry(main_prompt, presentation_prompt, model_name, conversation_template):
...
def main():
    ...
    # Temporary file path for storing intermediate results
    temp_file_path = f"data/{model_name_file_path}_answers_{course}_temp_{timestamp}.csv"

    # File path for completed indices
    completed_indices_file_path = f"data/indices_{model_name_file_path}_answers_{course}.csv"

    # Check if the completed indices file exists
    if os.path.exists(completed_indices_file_path):
        completed_indices_df = pd.read_csv(completed_indices_file_path)
        completed_indices = completed_indices_df['indices'].tolist()
        indices_to_do = [idx for idx in indices if idx not in completed_indices]
    else:
        completed_indices_df = pd.DataFrame(columns=['indices'])
        indices_to_do = indices
    ...
    print(f"Indices to do: {len(indices_to_do)}")
    try:
        for idx in tqdm(indices_to_do, desc="Processing indices", total=len(indices_to_do)):
            for explainer in explainer_list:
                ...
                for index, row in prompt_object.prompting_strategies.iterrows():
                    ...
                    response1, response2 = call_groq_model_with_retry(main_prompt, presentation_prompt, model_name, conversation_template)
            ...
            # After completing all explainers for the current idx, append the idx to the completed indices file
            completed_indices_df = pd.concat([completed_indices_df, pd.DataFrame({'indices': [idx]})], ignore_index=True)
            completed_indices_df.to_csv(completed_indices_file_path, index=False)
