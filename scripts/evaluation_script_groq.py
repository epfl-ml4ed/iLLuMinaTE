...
def generate_evaluation(row, model, questions, conversation_template, evaluation_template):
    while True:
        ...
        try:
            llm_class = chat_groq_model(model, 0.5)
            ...
            # Ensure the loop takes at least 15 seconds
...
def main():
    ...
    # %%
    answers = pd.read_csv(f'data/15_students_results/gpt4o_answers_{course}.csv')
    ...
    llm_class = chat_groq_model(model, 0.5, key)
    conversation = llm_class.define_chat_model(conversation_template)
    ...
    results_df.to_csv(f'data/{model}_evaluation_{course}.csv', index=False)
